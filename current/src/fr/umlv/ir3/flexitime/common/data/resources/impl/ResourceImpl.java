...
public abstract class ResourceImpl extends DataImpl implements IResource
{
    ...
    public Set<IBusy> getSetBusy()
    {
        if (setBusy == null)
        {
            ...
            if (this instanceof IDevice)
                l = LocalDataManager.getManager().getBusies(idData,
                        IDevice.class);
            if (this instanceof IGroup)
                l = LocalDataManager.getManager().getBusies(idData,
                        IGroup.class);
            if (this instanceof IRoom)
                l = LocalDataManager.getManager()
                        .getBusies(idData, IRoom.class);
    ...
    /**
     * Adds an unavailibility to the list. <code>resource.addBusy(busy)</code>
     * 

...
public class GroupManager extends AbstractManager
{
    ...
    {
        if(!(data instanceof IGroup)) return false;
        GroupStorage.save((IGroup) data);
    ...
    {
        if(parent instanceof IClass)return GroupStorage.get((IClass) parent);
        return null;
    }
    /** 
     * DOCME Description
    ...
    {
        if(!(data instanceof IGroup)) return false;
        GroupStorage.delete((IGroup) data);
    ...
    {
        if(!(data instanceof IGroup)) return false;
        GroupStorage.update((IGroup) data);

...
public class FlexiLDAP { 
	 ...
	 * Initialise les attributs selon les paramètres de l'UMLV
	 */
	public FlexiLDAP(){
	...
	public boolean createConnection(String name, String passwd){
		...
		env.put(Context.PROVIDER_URL, "ldap://"+serverLdap+":"+port+"/");   //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
	...
	public ArrayList getAttribute(String component, int type, String name){
	    ...
	    try {
	        ...
	        if(type==TYPE_USER) str = "uid="+name+","+pathUser;  //$NON-NLS-1$//$NON-NLS-2$
	        // Si c'est un groupe
	        else if(type==TYPE_GROUP) str = "cn="+name+","+pathGroup; //$NON-NLS-1$ //$NON-NLS-2$
	        else return null;

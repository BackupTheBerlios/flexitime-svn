...
public class FloorManager extends AbstractManager
{
    ...
    {
        if(!(data instanceof IFloor)) return false;
        FloorStorage.save((IFloor) data);
    ...
    {
        if(parent instanceof IBuilding)return FloorStorage.get((IBuilding) parent);
        return null;
    }
    /** 
     * DOCME Description
    ...
    {
        if(!(data instanceof IFloor)) return false;
        FloorStorage.delete((IFloor) data);
    ...
    {
        if(!(data instanceof IFloor)) return false;
        FloorStorage.update((IFloor) data);
